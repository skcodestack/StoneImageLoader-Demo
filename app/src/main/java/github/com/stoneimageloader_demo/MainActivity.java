package github.com.stoneimageloader_demo;
...
import github.com.stoneimageloader_demo.stoneimageloader.cache.DoubleCache;
...
public class MainActivity extends AppCompatActivity {
    ...
    StoneImageLoader imageLoader;
    private static final int EXPORT_PERMISSION_REQUEST_CODE =  0x00321;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listview = (GridView) findViewById(R.id.gl);
        ImageLoaderConfig.Builder builder = new ImageLoaderConfig.Builder();


        ImageLoaderConfig config = builder.setCachePolicy(new DoubleCache(this))
                .setFaildImageRes(R.drawable.miss_default_error)
                .setLoaderPolicy(new ReversePolicy())
                .setThreadCount(3)
                .setLoadingImageRes(R.drawable.loading)
                .build();


        imageLoader = StoneImageLoader.getInstance(config);

        StonePermission.with(this)
                .permissions(Manifest.permission.INTERNET,Manifest.permission.WRITE_EXTERNAL_STORAGE)
                .addRequestCode(EXPORT_PERMISSION_REQUEST_CODE)
                .request();


    }
    @PermissionFail(requestCode = EXPORT_PERMISSION_REQUEST_CODE)
    private void onfail(){
    ...
    @PermissionSuccess(requestCode = EXPORT_PERMISSION_REQUEST_CODE)
